...
- **PWA service worker — restart-to-update (no in-place reload).** Both apps use Blazor's `service-worker.published.js` with cache-name keyed on `assetsManifest.version`. The worker **does not** call `skipWaiting()` and is **never** activated in place — after a deploy the new worker pre-caches assets and then *waits*. The new version takes effect only on the **next fresh launch** of the app (when no client of the old worker remains). The page's registration script (in each app's `index.html`) detects the waiting worker and notifies the shared [AppUpdateBanner.razor](Tsump.Shared/Components/AppUpdateBanner.razor), which just *informs* the user ("Close the app and reopen to update") and offers a **best-effort "Close app now"** (`window.close()` via `closeAppForUpdate` — works for an installed desktop PWA, usually blocked on mobile, in which case the banner falls back to a manual-close hint). New pages deliberately have **no "Update now" / `controllerchange→reload`** path: the earlier auto-reload reloaded the page and risked corrupting data, since saves are read-modify-write of a whole collection across `await`s (see [SessionService.cs](Tsump/Services/SessionService.cs)). The SW **does** still keep a dormant `message`→`skipWaiting` handler purely for **backward compatibility** — an earlier deploy shipped an "Update now" button that posts `SKIP_WAITING`, and removing the handler left that button dead for users mid-transition. Don't remove it again. The banner and the JS bridge (`registerAppUpdateCallback` / `closeAppForUpdate`) must stay in sync across **both** repos.
