...
- **PWA service worker — controlled, user-triggered updates.** Both apps use Blazor's `service-worker.published.js` with cache-name keyed on `assetsManifest.version`. The worker **does not** call `skipWaiting()` — after a deploy the new worker pre-caches assets and then *waits*, leaving the running app on the old version. The page's registration script (in each app's `index.html`) detects the waiting worker and notifies the shared [AppUpdateBanner.razor](Tsump.Shared/Components/AppUpdateBanner.razor), which shows "Update now / Later". Only when the user clicks **Update now** does the page `postMessage('SKIP_WAITING')`; the worker then activates, `controllerchange` fires, and the page reloads — at a moment the user chose. This deliberately replaced the old auto-`skipWaiting` + `controllerchange→reload` behaviour, which reloaded mid-action and corrupted data (saves are read-modify-write of a whole collection across `await`s — see [SessionService.cs](Tsump/Services/SessionService.cs)). The banner, JS bridge (`registerAppUpdateCallback` / `applyAppUpdate`), and SW `message` handler must stay in sync across **both** repos.
