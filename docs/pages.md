...
- **Dual scoring config**: weekly defaults (`WeeklyStartingPoints` / `WeeklyUma{3,4}Players`) and tournament defaults (`TournamentStartingPoints` / `TournamentUma{3,4}Players`) are independent. A tournament can additionally have its own per-tournament overrides — those fall back to the tournament-defaults from settings only via `TournamentScoreContextResolver`'s null-coalesce. The Settings page only edits the club-wide defaults; the per-tournament overrides live on the tournament's own **Scoring Settings** page ([TournamentScoringSettings.razor](../Tsump/Pages/TournamentScoringSettings.razor), route `/tournament/{id}/scoring`, the **"Settings"** sidebar entry).
...
## Tournaments.razor / TournamentParticipants.razor / TournamentScoringSettings.razor

- **Overview edit ([Tournaments.razor](../Tsump/Pages/Tournaments.razor)).** The create-tournament card doubles as an edit form (`editId == null` ⇒ create, else edit name / short name / date / hanchans). Editing only triggers a warning + confirm (`ChangeHanchansWarning`) when the tournament **is already generated *and* the hanchan count actually changes** — that's the only edit that invalidates the assignments, so on confirm it clears `Sessions` + resets `IsGenerated`. Name/short-name/date edits save silently.
...
- **Manual number assignment ("Clear all numbers" / "Edit numbers").** Alongside Randomize, a confirm-gated **Clear all numbers** sets every `TournamentParticipant.Number` to **0 — the "not assigned yet" sentinel** — and switches the Nr. column to editable number inputs (`assigningNumbers`). **Edit numbers** enters the same mode without wiping anything, so a half-finished round can be resumed; both funnel through `EnterAssignMode`, which focuses the first player still at 0. Both also need the reset consent when the tournament is already generated, and both `ResetAssignments()` — entering the mode at all invalidates an existing assignment. That consent is now shared: `RequireResetConsent(action, warningKey)` parks the action and raises the inline alert (replacing six copies of the same block), with `resetWarningKey` carrying the reason — `ChangeNumbersWarning` for the number paths, `ChangeParticipantsWarning` otherwise — restored to the default by `ConfirmReset`/`CancelReset`. While assigning, rows sort *unnumbered-first, alphabetically by name*, then the numbered ones by number, so a player drops below the alphabetical list the moment they get a number. The field is `type="text"` + `inputmode="numeric"` (a `type=number` spinner is useless here, and `maxlength` — set to the digit count of N — only applies to text). Entries outside **1..participant-count**, and non-numeric ones, are dropped and the field snaps back: since the model never took the value, a plain value-diff can't restore the DOM, so `RejectNumberEntry` bumps `numberInputEpoch` (part of each input's `@key`) to force the inputs to be re-created, and re-focuses the field. Numbers commit on **Enter / Tab** (`OnNumberKeyDown`, reading the `@oninput`-tracked `numberEditText`) and on blur (`@onchange`, which reads `e.Value` instead — a blur-commit can land after the *next* field's focus handler has already re-pointed `numberEditText`). Commit is never per-keystroke, or the row would jump away mid-typing. **The two keys deliberately do different things after a successful commit** — see `CommitFollowUp` / `ApplyFollowUp`. **Enter** means *commit and show me the top of the list*: it drops the caret entirely (no field ends up focused) and scrolls the re-sorted topmost row to the top of the viewport, via `pendingScrollToTopRow` consumed in `OnAfterRenderAsync` → `scrollRowToTop`. **Tab** means *commit and keep typing*: `FocusNextUnnumbered` puts the caret in the next still-unnumbered player's box (walking the post-commit order forward with wrap-around, so it lands on the first remaining one), via `pendingFocusId` consumed in `OnAfterRenderAsync` → `focusRowInput`. Shift+Tab and blur-commits (`@onchange`) do neither. Rejected entries and an open duplicate prompt both `return` *before* the follow-up runs, so the field keeps the caret for correction. **The Enter blur has to happen *before* the commit re-renders the table** (`OnNumberKeyDown` awaits `blurActiveElement` *before* calling `CommitNumber`), and this is subtle enough to be worth spelling out. Blazor's renderer **re-applies focus to an element it moves while that element is focused** — `updateComponent` calls a plain `.focus()` with no `preventScroll` — and because bootstrap-reboot sets `:root { scroll-behavior: smooth }` that starts a *smooth* scroll chasing the input, which has just re-sorted **down** into the numbered block. Blurring after the render is too late: `blur()` does not cancel a scroll already in flight, and an instant scroll only cancels one when it actually moves the viewport. That produced a bug that **alternated every other commit** — when the view was already at the target the corrective scroll was a no-op, cancelled nothing, and the smooth animation ran on to the bottom of the page. Blurring first means there is no focused element for Blazor to restore, so the competing scroll is never started; `scrollRowToTop` also aborts any in-flight smooth scroll before positioning, as belt and braces. Because the blur fires the `@onchange` commit first, `OnNumberKeyDown` snapshots `numberEditText` into a local *before* blurring — on a rejected entry that commit re-points `numberEditText` at the participant's existing number. Tab's *native* move has to be suppressed or it stacks on top of ours and skips a row: the inputs carry `data-tab-handled`, and a capture-phase `keydown` listener in [index.html](../Tsump/wwwroot/index.html) calls `preventDefault` for plain Tab on those fields (Blazor's `@onkeydown:preventDefault` is fixed at render time, so it can't single out Tab without also blocking typing). Shift+Tab is deliberately left alone — it commits, and the browser does the backwards move. Typing a number another participant already holds shows an inline `alert-warning` naming that participant; **Continue** hands the number over and puts the previous holder back at 0 (into the alphabetical list), **Cancel** reverts the field. `NumberDisplay` keeps the typed value visible while the prompt is open so the warning matches the field. Side effects while assigning: a newly added participant gets `Number = 0` instead of `Max+1`, and `RemoveParticipant` skips its 1..n compaction (it would overwrite the numbers just handed out). An alert counts how many are still unnumbered (`alert-info` while assigning, `alert-warning` + the `NumbersMissing` reason outside the mode). The green **Save** button leaves the mode. Like Randomize, clearing runs through `NeedsResetConfirmation` + `ResetAssignments`. Rows outside the mode render an empty Nr. cell for 0.
- **Entering an edit mode focuses a field, and scrolls its row into view.** Both name and number modes route through the shared `pendingFocusId` (+ `pendingFocusSelect`) consumed in `OnAfterRenderAsync`, which calls `focusRowInput(id, select)` in [index.html](../Tsump/wwwroot/index.html): focus with `preventScroll`, optional select-all (number boxes — so you type straight over; name boxes leave the text alone), then `scrollIntoView({block:'center'})` inside a `requestAnimationFrame`. Both details matter, and getting either wrong shows up the same way — the view trailing one row behind the caret while numbers are typed quickly. The scroll must be **instant**, and `behavior: 'instant'` has to be passed explicitly: the default `'auto'` follows CSS `scroll-behavior`, and `bootstrap-reboot.css` sets `:root { scroll-behavior: smooth }`, so omitting it silently keeps the animation — which is still running when the next row is committed, so each step starts from a mid-flight position and never catches up. The rows also re-sort as the number lands, so the scroll has to come after that layout (hence the rAF). Centering unconditionally is deliberate — the target is normally the table's top row, so it stays put and the list advances underneath it. **Edit names** and **Generate players** land on the first row with a blank name (`FocusFirstNameInput`); the number modes land on the first row still at 0. The Enter-commit scroll is the one case that is **top-aligned** rather than centered (`block: 'start'`), since the point is to show the top of the list: each `<tr>` carries `id="participant-row-{id}"` + `.participant-row`, whose `scroll-margin-top: 4rem` keeps the row clear of MainLayout's `.top-row` (sticky, 3.5rem, on wide screens). It is instant and rAF-deferred for the same two reasons `focusRowInput` is.
- **Unnumbered participants block table assignment.** `TournamentDetail.HasUnnumberedParticipants` (any `Number == 0`) disables *both* the initial **Generate all hanchans** and the **Regenerate** button, and `GenerateSessions` early-returns as a backstop. A disabled button swallows its `title` tooltip in most browsers, so the reason is also on the page: `UnnumberedNotice` (an `alert-warning` with the `NumbersMissing` text and a link to Participants) renders next to whichever generate button is showing. `GenerateBlockedReason()` still fills the `title` — `NeedAtLeast13` first, then `NumbersMissing`. Hand-assigned numbers then **survive** generation — `ApplyUniversalStartPositions` relabels seats rather than people, so table 1 still holds numbers 1-4 while every player keeps the number they drew. See the numbering contract in [domain.md](domain.md#table-assignment-tournament).
