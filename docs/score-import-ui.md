...
5. On reaching `Preview`/`AlreadyScored` the panel fires `OnPreviewShown(ScoringResult)`. Both host pages use it to **scroll the table that will be updated into view** (via their existing `table-scroll-anchor` ids + `scrollToElement`): WeeklySessionPage switches to the target hanchan first if the result belongs to a sibling hanchan of the same date (staying on the Scores tab); TournamentDetail expands the target hanchan on the per-hanchan view, or scrolls the table group on the per-table view. If the target isn't on the current page (other date / other tournament / regenerated table), the handlers do nothing — the preview's label already identifies the target.

   So the preview never scrolls out of view, entering it also flips the panel into a **floating overlay** (`floating` field → `.import-panel-floating`: fixed, top-centred, `z-index` above the sticky table-nav strip) — the page scrolls to the table *underneath* the pinned preview, and `scrollToElement` reserves the overlay's height when positioning the target. The panel *stays* floating through `Applied` and the auto-continued capture states (camera restart / hand-scanner re-arm stay visible even with the page scrolled down at a table); it drops back into normal flow on any return to neutral (`ReturnToNeutralCore`: cancel, close, the post-image continuation) — sticky wasn't an option because TournamentDetail hosts the panel inside a flex action row it can't escape.
