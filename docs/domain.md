...
**Hanchan start times are stored twice, on purpose.** `Tournament.StartTimes` (`List<TimeSpan?>?`, index i = hanchan i+1) is the tournament's *override*: null means "follow `ClubSettings.TournamentStartTimes`". `TournamentSession.StartTime` is the copy every consumer actually reads (printouts, guidesheets, scoring invites, rankings) and only exists once the tournament is generated. [HanchanStartTimes](../Tsump/Services/HanchanStartTimes.cs) keeps the two in step and is the **only** place either should be written: `Effective` resolves the times for hanchans 1..`SessionCount` — override, else the time already stamped on that session, else the club default — `ApplyToSessions` stamps them onto the sessions (called by `DoGenerate`), `Set` writes one hanchan, `Reset` drops the override. The stamped-session fallback is what keeps tournaments generated *before* `StartTimes` existed showing their real times rather than today's club defaults; it can't swallow a regenerate (fresh sessions carry no time), and `Reset` clears the stamps before re-applying or it would just restore what it was resetting. `Set` materialises the whole list from the effective times on first use — so the first edit freezes today's club defaults for the *other* hanchans too, rather than leaving them tracking a setting that may change later.
...
2. **3-player duty fairness**: candidates are ranked on the **post-assignment ratio** `(threeCount + 1) / (attended + 1)` — the ratio a player *would* have after taking duty. This must stay consistent with `ScoreThreePlayerFairness` and makes first-timers (attendance 0) the *least* attractive candidates (ratio 1.0). The pre-2026-07 code ranked on the historical ratio, which mapped "no history" to 0.0 and *guaranteed* newcomers a 3-player seat. `Member.ExtraThreePlayerTableCount` seeds the count for players who owe extra duty. It is a **`double`**, not an int: 0 is neutral, positive means "treat as having already had that much 3-duty", negative the reverse. Hand-editable on the member form (step 0.1, spinner capped at ±0.5, but the form is `novalidate` so a deliberate out-of-range correction can still be typed; a `bi-question-circle` next to the label opens the same style of explainer the History page's % column uses, `ExtraThreePlayerTablesExplain`, covering what positive/negative do and why to stay inside ±0.5), and rewritten wholesale by the start-of-round reset, which carries the outgoing round's standing into it (see [pages.md](pages.md#settingsrazor)). Because it is fractional, every count derived from it is a `double` too — `CountThreePlayerAssignments` and the `threePlayerCounts` dictionaries here, `PlayerStat.ThreePlayerCount` on the History page, and the `Member` record in the Simulation bench (an `int` there would throw on any export taken after a round change). Attendance stays an `int`, so the attendance tiers and the `1e-9` ratio epsilon in `SelectTierSubset` are unaffected. Ties at the cutoff ratio are broken by attendance (highest first); when a tier of players tied on *both* ratio and attendance only partly fits the remaining slots, duty goes to the **subset with the lowest mutual meeting cost** — exhaustive over all C(n,k) subsets (`SelectTierSubset`; greedy fallback above a combination cap for pathological cases like a first-ever session), random among equal-best subsets. The earlier greedy one-at-a-time pick was effectively a lottery when everyone was tied: its first pick scored against an empty selection (all zeros), and since every tied subset has the same fairness score, the attempt loop's early exit meant only one candidate trio was ever evaluated.
...
**Ranking compensation for missed hanchans** (`AbsentScoringMode` + correction, [ClubSettings.cs](../Tsump/Models/ClubSettings.cs)). A dropped-out player has fewer played hanchans than everyone else; `RankingTable.BuildRankings` applies the configured policy — but **only** for hanchans where the player is in `AbsentPlayerIds`. A merely not-yet-scored table also yields an empty cell, and substituting those would corrupt mid-tournament standings, so they stay `—`. Three modes (radio buttons, not a dropdown): **Zero** (default; a missed hanchan counts 0 — identical totals to the pre-feature behaviour), **Average** (the mean of the player's *played* hanchans, recomputed on every render, so the substituted value shifts as results come in — by design), or **Excluded** (`RankingEntry.Excluded`: the player leaves the ranking entirely and is listed on an unranked, greyed line *below* it — actual scores in played columns, `—` for missed ones, no substitution and the rank numbering skips them). A signed **correction** (×1000) is added per missed hanchan on top of Zero/Average; it doesn't apply to Excluded, and the input is disabled there. Substituted cells render italic/muted on screen and in the HTML export, and with a `*` suffix in the plain-text export. Configured club-wide on Settings → Tournament, overridable per tournament on its Scoring Settings page (`Tournament.AbsentScoringMode` / `AbsentCorrection`, null = club default, same pattern as the uma overrides). Weekly rankings pass no policy and are untouched.

Marking absent is **organizer-only, tournaments-only and per table** — there is no cascade to the player's later hanchans, and the scoring app has no notion of it at all (it just receives an ordinary 3-player invite). It lives on the **table-assignment card** in [TournamentDetail.razor](../Tsump/Pages/TournamentDetail.razor)'s Assignments view — the same card that hosts swap/drag — and is deliberately **hidden until a player is selected** (the same click-select gesture swap uses): a rarely-used action shouldn't put a button on every player row. Selecting a player reveals a `person-dash` button on that row; an absent player renders struck-through with an always-visible ⟲ restore button (absent rows can't be selected, so the undo can't hide behind selection). Not gated on any scoring setting. Constraints: only while the table has four active players (two real + Mr. X isn't playable — a second dropout must be fixed by reseating) and only while the table has no scores (scores are seat-bound). The invite QR needs no special handling — it's rebuilt every time the modal opens, so it always carries the current active players.
